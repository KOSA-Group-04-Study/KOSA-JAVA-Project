...
public class ReservationController {
    ...
    public  static void start(User user) {
        ...
        try {
            ...
            Map<Integer, MovieScreaningInfo> scheduleNumbersMap = getAndPrintMovieSchedules(data);      //영화 상영 스케쥴에 넘버링한 자료구조(메인 데이터 관리)
    ...
    //입력받은 날짜의 상영정보를 보여준다
    private static Map<Integer, MovieScreaningInfo> getAndPrintMovieSchedules(Map<String, Map<Movie, Schedule[][]>> data) {
        Map<Integer, MovieScreaningInfo> scheduleNumbersMap = new HashMap<>(); //날짜 변경에 따른 초기화
    ...
    //3.사용자에게 원하는 영화 스케쥴을 입력받는다.
    private static Integer inputChoiceSchedule(Map<Integer, MovieScreaningInfo> scheduleNumbersMap) {
        do {
    ...
    private static void updateScheduleInfo(Map<Integer, MovieScreaningInfo> scheduleNumbersMap, Integer choiceSchedule) {
        //좌석 정보 업데이트
